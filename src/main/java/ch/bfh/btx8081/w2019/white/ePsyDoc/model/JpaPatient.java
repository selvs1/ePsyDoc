...
public class JpaPatient {
	...
	public void setUp() {
		...
		if (createNewDoctorEntries) {
			String[][] dataFeed = { { "Figo", "Luis", "Dorfmattweg 8z", "05.09.1990", "m", "3110" },
			...
			for (int i = 0; i < dataFeed.length; i++) {
				...
				patient.setDate(new Date(Integer.parseInt(dataFeed[i][3].substring(6)),
						Integer.parseInt(dataFeed[i][3].substring(3, 5)),
						Integer.parseInt(dataFeed[i][3].substring(0, 2))));
